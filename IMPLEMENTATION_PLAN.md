# Frontend Task Integration - Implementation Plan
...
- Functions:
  ...
  async function uploadFile(signedUrl: string, file: File): Promise<{ key: string }>
  async function registerAttachment(key: string, file: File): Promise<{ id: string }>
  ```
